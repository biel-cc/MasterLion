...
describe('UploadService', () => {
  ...
  describe('uploadToServerS3', () => {
    ...
    it('should report progress during upload', async () => {
      ...
      expect(lambdaClient.upload.createS3PreSignedUpload.mutate).toHaveBeenCalledWith({
    ...
    it('should forward required presigned upload headers to XMLHttpRequest', async () => {
      const xhr = new XMLHttpRequest();
      vi.spyOn(xhr, 'addEventListener').mockImplementation((event, handler) => {
        if (event === 'load') {
          // @ts-expect-error - mock implementation
      ...
      expect(xhr.open).toHaveBeenCalledWith('PUT', mockPreSignUrl);
