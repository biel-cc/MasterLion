...
const provisionWecomSessionAccount = async (
  ...
  const wecomAccount = await serverDB.query.account.findFirst({
    where: (account, { and, eq }) =>
      and(eq(account.providerId, 'wecom'), eq(account.userId, session.userId as string)),
  });

  if (!wecomAccount) return;

  await provisionWecomLoginAccount({
    account: {
      accountId: wecomAccount.accountId,
      providerId: wecomAccount.providerId,
      userId: wecomAccount.userId,
    },
    context,
  });
...
export function defineConfig(customOptions: CustomBetterAuthOptions) {
  ...
  const options = {
    ...
    databaseHooks: {
      account: {
        create: {
          after: async (account, context) => {
            if (account.providerId !== 'wecom') return;
      ...
      session: {
        create: {
          after: provisionWecomSessionAccount,
        },
      },
      user: {
        create: {
