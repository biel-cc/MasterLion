...
import { Flexbox, type GenericItemType, Icon, Skeleton, Tag, Text, Tooltip } from '@lobehub/ui';
...
import { getTopicPlatformIcon } from './platformIcon';
...
export interface TopicItemProps {
  ...
  threadId?: string;
  ThreadListComponent?: ComponentType<{ topicId: string }>;
  title: string;
}
...
const TopicItem = memo<TopicItemProps>(
  ({
    ...
    // NavItem's legacy root ContextMenu and base-ui DropdownMenu accept the same item/divider
    // shapes but currently publish separate nominal item types during the base-ui migration.
    const contextMenuItems = dropdownMenu as () => GenericItemType[];
    ...
    return (
      <Flexbox data-testid="topic-item" style={{ position: 'relative' }}>
        <NavItem
          ...
          icon={(() => {
            ...
            if (metadata?.bot?.platform) {
              const ProviderIcon = getTopicPlatformIcon(metadata.bot!.platform);
              if (ProviderIcon) {
        ...
        {shouldShowThreadList && ThreadListComponent && (
          <Suspense
            fallback={
          ...
          >
            <ThreadListComponent topicId={id} />
          </Suspense>
        )}
      </Flexbox>
    );
