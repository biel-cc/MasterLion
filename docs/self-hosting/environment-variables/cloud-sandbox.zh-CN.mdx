...
云端沙箱用于内置的代码执行、Shell 命令、文件操作和文件导出工具。只有服务端显式配置提供方后才可用；部署可以选择 Market，也可以使用兼容 Onlyboxes 的自托管提供方。
...
未设置该变量时，服务端会把云端沙箱报告为不可用，并继续拒绝沙箱工具调用。

### `MARKET_BASE_URL`

- 类型：可选
- 描述：Market 服务的基础 URL。使用官方 Market 时无需配置；仅当你需要连接自托管或专用 Market 服务时设置。
- 默认值：`https://market.aihub.bielcrystal.com`
- 示例：`https://market.example.com`

### `MARKET_TRUSTED_CLIENT_ID`

- 类型：可选
- 描述：Market Trusted Client 的客户端 ID，用于让 Masterino 服务端代表当前用户调用 Market 能力。需要由对应 Market 服务登记。
- 默认值：-
- 示例：`lobechat-com`
...
- 类型：可选
- 描述：Masterino 签发的每个 Onlyboxes JIT Token 的有效期。
- 默认值：`1800`
- 示例：`900`
...
如果该 Market 服务要求 Masterino 服务端代表当前用户调用沙箱、凭据或技能能力，还需要配置 Trusted Client：
...
配置的 Onlyboxes worker 需要暴露 `terminalExec` 和 `terminalResource`。Masterino 使用 `terminalExec` 作为 Shell 命令、代码执行和文件操作的兼容层，并使用 `terminalResource` 通过预签名上传 URL 导出文件。
...
Masterino 推荐让 `WORKER_PYTHON_EXEC_DOCKER_IMAGE` 与 `WORKER_TERMINAL_EXEC_DOCKER_IMAGE` 指向同一个不可变 digest。官方部署包的均衡版基线还包含 pandas/NumPy/SciPy、matplotlib/seaborn、PyArrow/Polars/DuckDB、openpyxl/XlsxWriter、python-docx/python-pptx、pypdf/ReportLab、OfficeCLI、LibreOffice、Pandoc、Poppler，以及 Noto CJK、Carlito、Caladea 字体。

核心依赖应预装并固定版本。运行时安装只能访问执行项目和版本白名单的内部 Python 镜像代理；不要允许沙箱直连公网 PyPI，也不要把代理凭据烘焙进镜像。

Onlyboxes worker 默认 capacity 不适合 30 个并发任务。生产部署需要显式配置 `WORKER_TERMINAL_EXEC_MAX_INFLIGHT`、`WORKER_TERMINAL_RESOURCE_MAX_INFLIGHT`、`WORKER_TERMINAL_MAX_ACTIVE_SESSIONS` 以及单容器 CPU、内存和 PID 限制，并通过多 Worker 节点保留故障余量。
...
<Callout type={'info'}>
  文件导出仍会把沙箱内产物写入 Masterino 配置的 S3 存储。如果用户需要下载沙箱生成的文件，请同时配置 S3
  相关环境变量。
</Callout>
