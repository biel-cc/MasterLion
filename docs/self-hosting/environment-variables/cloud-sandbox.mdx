...
Cloud Sandbox powers the built-in code execution, shell command, file operation, and file export tools. It remains unavailable until a server-side provider is explicitly configured. Deployments can use Market or an Onlyboxes-compatible self-hosted provider.
...
- Default: `-` (Cloud Sandbox disabled)
- Example: `onlyboxes`
...
When this variable is unset, the server reports Cloud Sandbox as unavailable and continues to reject sandbox tool calls.
...
- Default: `https://market.aihub.bielcrystal.com`
- Example: `https://market.example.com`

### `MARKET_TRUSTED_CLIENT_ID`

- Type: Optional
- Description: Market Trusted Client ID used by the Masterino server to call Market capabilities on behalf of the current user. It must be registered by the target Market service.
- Default: -
...
- Type: Optional
- Description: Lifetime of each Onlyboxes JIT token minted by Masterino.
- Default: `1800`
- Example: `900`
...
To use the official Market sandbox, configure it explicitly:
...
If that Market service requires the Masterino server to call sandbox, credential, or skill capabilities on behalf of the current user, also configure Trusted Client credentials:
...
The configured Onlyboxes worker should expose `terminalExec` and `terminalResource`. Masterino uses `terminalExec` as the compatibility layer for shell commands, code execution, and file operations, and uses `terminalResource` for file export through a pre-signed upload URL.
...
Masterino recommends pointing `WORKER_PYTHON_EXEC_DOCKER_IMAGE` and `WORKER_TERMINAL_EXEC_DOCKER_IMAGE` at the same immutable digest. The balanced runtime in the official deployment bundle also includes pandas/NumPy/SciPy, matplotlib/seaborn, PyArrow/Polars/DuckDB, openpyxl/XlsxWriter, python-docx/python-pptx, pypdf/ReportLab, OfficeCLI, LibreOffice, Pandoc, Poppler, and Noto CJK, Carlito, and Caladea fonts.

Preinstall and pin core dependencies. Runtime installation should only use an internal Python proxy that enforces project and version allowlists. Do not allow public PyPI egress or bake proxy credentials into the image.

The default Onlyboxes worker capacity is not suitable for 30 concurrent jobs. Production deployments should explicitly configure `WORKER_TERMINAL_EXEC_MAX_INFLIGHT`, `WORKER_TERMINAL_RESOURCE_MAX_INFLIGHT`, `WORKER_TERMINAL_MAX_ACTIVE_SESSIONS`, and per-container CPU, memory, and PID limits, then retain failure capacity across multiple worker nodes.
...
<Callout type={'info'}>
  File export still writes the exported artifact to the configured Masterino S3 storage. Configure the
