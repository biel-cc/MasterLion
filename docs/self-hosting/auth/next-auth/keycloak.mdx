---
title: Configuring Keycloak Authentication Service in Masterino
description: >-
  Learn how to configure the Keycloak authentication service in Masterino,
...
# Configuring Keycloak Authentication Service in Masterino
...
<Callout type={'tip'}>
  If you want to privately deploy Keycloak, we recommend using it together with Masterino via Docker
...
- Your Masterino database version IP/port is `http://LobeHub_IP:3210`.
- Your privately deployed Keycloak domain is `http://KEYCLOAK_IP:8080`.

If you deploy using a public network, this guide assumes:

- Your Masterino database version domain is `https://lobe.example.com`.
...
<Steps>
  ...
  1. Create a new Realm
     ...
     - Enter a name, such as "Masterino", then click "Create"
  ...
  2. Create a Client
     ...
     - On the "Login settings" page:
       ...
       - Web origins: Add your Masterino domain or IP
       - Click "Save"
  ...
  Set the obtained client ID and client secret as `AUTH_KEYCLOAK_ID` and `AUTH_KEYCLOAK_SECRET` in the Masterino environment variables.

  Configure the Masterino environment variable `AUTH_KEYCLOAK_ISSUER` as:

  - `http://localhost:8080/realms/Masterino` for local development environment
  - `http://KEYCLOAK_IP:8080/realms/Masterino` for privately deployed Keycloak on a local network
  - `https://lobe-auth-api.example.com/realms/Masterino` for Keycloak deployed in a public environment

  When deploying Masterino, you need to configure the following environment variables:
  ...
  | `NEXT_AUTH_SSO_PROVIDERS` | Required | Select the single sign-on provider for Masterino. For Keycloak, fill in `keycloak`.                                                                                                        |
...
<Callout type={'info'}>
  After successful deployment, users will be able to authenticate through Keycloak and use Masterino.
</Callout>
