...
export interface TopicDocumentSummary extends Pick<
  DocumentItem,
  | 'createdAt'
  | 'description'
  | 'fileType'
...
export interface TopicDocumentPlan extends Pick<
  DocumentItem,
  | 'content'
  | 'createdAt'
  | 'description'
...
export class TopicDocumentModel {
  ...
  findLatestPlanByTopicId = async (topicId: string): Promise<TopicDocumentPlan | null> => {
    const [result] = await this.db
      ...
      .orderBy(desc(topicDocuments.createdAt), desc(documents.updatedAt), desc(documents.id))
      .limit(1);
