...
const getDocumentResourceType = (fileType?: null | string) =>
  fileType === CUSTOM_FOLDER_FILE_TYPE ? 'folder' : 'document';

const assertResourceAccess = async (
  ctx: any,
  resource: ResourceRef,
  permission: ResourcePermission,
  owner?: { ownerUserId?: null | string; ownerWorkspaceId?: null | string },
  resourceChain?: ResourceRef[],
) => {
  const scope =
    owner && resourceChain ? undefined : await resolveResourceAclScope(ctx.serverDB, resource);
  const resolvedOwner = {
    ownerUserId: owner?.ownerUserId ?? scope?.ownerUserId,
    ownerWorkspaceId: owner?.ownerWorkspaceId ?? scope?.ownerWorkspaceId,
  };
  ...
  return resourceAclService.assertCan({
    ...
    ownerWorkspaceId: resolvedOwner.ownerWorkspaceId,
    permission,
    platformRole: getPlatformRole(ctx),
    resource: scope?.resource ?? resource,
    resourceChain: resourceChain ?? scope?.resourceChain,
    userId: ctx.userId,
    workspaceId: ctx.workspaceId ?? null,
  });
};
...
const assertFileAccess = async (
  ...
  id: string,
  permission: ResourcePermission,
  file?: Awaited<ReturnType<FileModel['findById']>>,
) => {
  const row = file ?? (await ctx.fileModel.findById(id));
  const resource = { resourceId: id, resourceType: 'file' as const };
  const scope = await resolveResourceAclScope(ctx.serverDB, resource);

  return assertResourceAccess(
    ctx,
    scope.exists ? scope.resource : resource,
    permission,
    {
      ownerUserId: scope.ownerUserId ?? row?.userId,
      ownerWorkspaceId: scope.ownerWorkspaceId ?? row?.workspaceId,
    },
    scope.resourceChain,
  );
};

const assertKnowledgeQueryAccess = async (
  ctx: any,
  input: {
    knowledgeBaseId?: string;
  ...
  if (input.knowledgeBaseId) {
    await assertResourceAccess(
      ctx,
      { resourceId: input.knowledgeBaseId, resourceType: 'knowledge_base' },
      permission,
    );
  }
  ...
  const parent =
    (await ctx.documentModel.findBySlug(input.parentId)) ??
    (await ctx.documentModel.findById(input.parentId));
  if (!parent) return;
  ...
  await assertResourceAccess(
    ...
    permission,
    { ownerUserId: parent.userId, ownerWorkspaceId: parent.workspaceId },
  );
};
...
export const fileRouter = router({
  ...
  createFile: fileProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await assertKnowledgeQueryAccess(
        ctx,
        { knowledgeBaseId: input.knowledgeBaseId, parentId: resolvedParentId },
        'write',
      );
  ...
  findById: fileProcedure
    ...
    .query(async ({ ctx, input }) => {
      const item = await FileModel.getFileById(ctx.serverDB, input.id);
      if (!item) throw new TRPCError({ code: 'BAD_REQUEST', message: 'File not found' });
      await assertFileAccess(ctx, input.id, 'read', item);
  ...
  getKnowledgeItems: fileProcedure.input(QueryFileListSchema).query(async ({ ctx, input }) => {
    await assertKnowledgeQueryAccess(ctx, { knowledgeBaseId: input.knowledgeBaseId }, 'read');
  ...
  removeFile: fileProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      await assertFileAccess(ctx, input.id, 'manage', existingFile);
  ...
  updateFile: fileProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await assertFileAccess(ctx, id, 'write');
