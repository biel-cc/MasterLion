...
const buildMockFileAccessUrl = ({ id }: { id: string }) => `https://aihub.bielcrystal.com/f/${id}`;
...
const routerMocks = vi.hoisted(() => {
  ...
  return {
    ...
    mockFileModelGetFileById: vi.fn(),
    mockResourceDocumentFindFirst: vi.fn(),
    mockResourceFileFindFirst: vi.fn(),
    mockResourceKnowledgeBaseFileFindFirst: vi.fn(),
    serverDB: {
      query: {
        documents: { findFirst: vi.fn() },
        files: { findFirst: vi.fn() },
        knowledgeBaseFiles: { findFirst: vi.fn() },
      },
      select: vi.fn(() => ({
        from: vi.fn(() => ({
          where: vi.fn(() => ({
...
function createCallerWithCtx(partialCtx: any = {}) {
  ...
  const ctx = {
    serverDB: {
      query: {
        documents: { findFirst: routerMocks.mockResourceDocumentFindFirst },
        files: { findFirst: routerMocks.mockResourceFileFindFirst },
        knowledgeBaseFiles: { findFirst: routerMocks.mockResourceKnowledgeBaseFileFindFirst },
      },
      select: vi.fn(() => ({
        from: vi.fn(() => ({
          where: vi.fn(() => ({
...
vi.mock('@/envs/app', () => ({
  appEnv: {
    APP_URL: 'https://aihub.bielcrystal.com',
  },
}));
...
vi.mock('@/database/models/file', () => ({
  FileModel: Object.assign(
    ...
    { getFileById: routerMocks.mockFileModelGetFileById },
  ),
}));
...
describe('fileRouter', () => {
  ...
  beforeEach(() => {
    ...
    routerMocks.mockFileModelGetFileById.mockResolvedValue(mockFile);
    routerMocks.mockResourceFileFindFirst.mockResolvedValue(mockFile);
    routerMocks.mockResourceDocumentFindFirst.mockResolvedValue(undefined);
    routerMocks.mockResourceKnowledgeBaseFileFindFirst.mockResolvedValue(undefined);
    routerMocks.serverDB.query.files.findFirst.mockResolvedValue(mockFile);
    routerMocks.serverDB.query.documents.findFirst.mockResolvedValue(undefined);
    routerMocks.serverDB.query.knowledgeBaseFiles.findFirst.mockResolvedValue(undefined);
  ...
  describe('findById', () => {
    it('should throw error when file not found', async () => {
      routerMocks.mockFileModelGetFileById.mockResolvedValue(null);
    ...
    it('should return proxy URL format ${APP_URL}/f/:id', async () => {
      ...
      expect(result.url).toBe('https://aihub.bielcrystal.com/f/test-id');
    });
  });
  ...
  describe('getFiles', () => {
    ...
    it('should return proxy URL format ${APP_URL}/f/:id for each file', async () => {
      ...
      expect(result[0].url).toBe('https://aihub.bielcrystal.com/f/file-1');
      expect(result[1].url).toBe('https://aihub.bielcrystal.com/f/file-2');
    });
  });
  ...
  describe('getKnowledgeItems', () => {
    ...
    it('should return knowledge items with files and documents', async () => {
      ...
      expect(result.items[0]).toMatchObject({
        ...
        id: 'file-1',
        sourceType: 'file',
        url: 'https://aihub.bielcrystal.com/f/file-1',
      });
