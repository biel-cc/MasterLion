    ...
    try {
      ...
      const assistant = {
        ...
        homepage:
          ...
          `https://aihub.bielcrystal.com/discover/assistant/${(data as any).identifier}`,
    ...
    try {
      ...
      const items: DiscoverAssistantItem[] = (data.items || []).map((item: any) => {
        ...
        return {
          ...
          homepage: `https://aihub.bielcrystal.com/discover/assistant/${item.identifier}`,
          identifier: item.identifier,
          installCount: item.installCount,
  ...
  getUserInfo = async (params: {
    ...
    try {
      ...
      const transformedAgents: DiscoverAssistantItem[] = (agents || []).map((agent: any) => ({
        ...
        homepage: `https://aihub.bielcrystal.com/discover/assistant/${agent.identifier}`,
      ...
      const transformedAgentGroups = (agentGroups || []).map((group: any) => ({
        ...
        homepage: `https://aihub.bielcrystal.com/discover/group_agent/${group.identifier}`,
      ...
      const transformedPlugins: DiscoverPluginItem[] = (plugins || []).map((plugin: any) => ({
        ...
        homepage: `https://aihub.bielcrystal.com/discover/plugin/${plugin.identifier}`,
