 ...
 *
 * Account model: each `(Masterino user, platform, tenant_id)` triple has at
 * most one row in `messenger_account_links`, so a single Masterino user can
...
export class MessengerRouter {
  ...
  private buildCommands(): MessengerCommand[] {
    return [
      {
        description: 'Bind your account to Masterino',
        handler: async (ctx) => {
          ...
          if (ctx.link) {
            await ctx.reply(
              'Your account is already linked to Masterino. Send /agents to switch the active agent, or /new to start a fresh conversation.',
            );
            return;
          }
          ...
          if (!ctx.isDM && !canEphemeralInChannel) {
            await ctx.reply('Check your DM with Masterino for the link button.');
          }
        },
        name: 'start',
      },
      ...
      },
      {
        ...
        handler: async (ctx) => {
          ...
          if (!ctx.thread) {
            await ctx.reply(
              'Open your direct message with the Masterino bot and send `/stop` there.',
            );
            return;
          }
      ...
      },
      {
        ...
        handler: async (ctx) => {
          // Feedback is tied to a Masterino account so the team can follow up;
   ...
   * which Masterino context a conversation runs in is the active agent's scope;
  ...
  private scopeSwitchedText(scopeName: string, defaultAgentTitle?: string): string {
    return defaultAgentTitle
      ...
      : `Switched to ${scopeName}. No agents here yet — create one in Masterino, then /agents.`;
  }
  ...
  private async handleAppHomeOpened(
    ...
    try {
      ...
      const text = activeAgentName
        ? `Welcome to Masterino! Your active agent is *${activeAgentName}*. Send a message to chat, or use \`/agents\` to switch.`
        : 'Welcome to Masterino! Send `/agents` to pick an active agent and start chatting.';
  ...
  private async dispatchToAgent(
    ...
    // Masterino user; the dispatch only fires for the linked sender. So
