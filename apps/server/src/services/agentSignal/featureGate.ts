...
export interface AgentSelfIterationCapabilityInput {
  ...
  /**
   * Whether the agent is the product-owned Masterino agent.
   */
  isLobeAiAgent: boolean;
}

/**
 * Checks whether a slug belongs to Masterino.
 *
 * Use when:
 * - Agent Signal must special-case the product-owned default assistant
 * - Code must avoid treating every virtual/builtin agent as Masterino
 *
 * Expects:
 ...
 * - Masterino should be managed by feature flag instead of stored chat config
 *
 * Expects:
 * - `isAgentSelfIterationFeatureEnabled` already includes the user-level feature-flag result
 *
 * Returns:
 * - `true` for Masterino when the feature flag is enabled
 * - `true` for non-Masterino only when `agentSelfIterationEnabled` is true
 */
