...
describe('agentSignal feature gates', () => {
  it('recognizes only the inbox builtin slug as Masterino', () => {
  ...
  it('enables Masterino self-iteration when the feature flag is enabled', () => {
    expect(
      resolveAgentSelfIterationCapability({
  ...
  it('keeps non-Masterino agents behind agentSelfIterationEnabled', () => {
    expect(
      resolveAgentSelfIterationCapability({
        agentSelfIterationEnabled: true,
        isAgentSelfIterationFeatureEnabled: true,
