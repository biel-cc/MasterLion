...
vi.mock('../device/register', () => ({
  ...
  resolveDeviceIdentity: vi.fn().mockReturnValue({ deviceId: 'mock-device-id', userId: 'test-user' }),
}));
...
describe('connect command', () => {
  ...
  it('should connect to gateway', async () => {
    ...
    expect(log.info).toHaveBeenCalledWith(expect.stringContaining('Masterino CLI'));
  });
  ...
  it('should pass the resolved serverUrl to GatewayClient', async () => {
    ...
    expect(clientOptions.serverUrl).toBe('https://aihub.bielcrystal.com');
  });
